...
public class App extends JFrame {
    ...
    private String[] buttonsNames = {"Home", "Book an Appointment", "Medical Records", "View Assigned Room", "Make Payments"};
    ...
    public App(int patientID, String firstName) {
        ...
        ActionListener leftButtonClicked = e -> {
            ...
            if (from == buttons[0]) {
                ...
                right.repaint();
                right.revalidate();
                right.add(new BookAppointmentFrame(patientID, firstName));
                right.repaint();
                right.revalidate();
                ...
                right.repaint();
                right.revalidate();
                right.add(new ViewMedicalRecordsFrame(patientID, firstName));
                right.repaint();
                right.revalidate();
                ...
                right.repaint();
                right.revalidate();
                right.add(new ViewAssignedRoomFrame(patientID, firstName));
                right.repaint();
                right.revalidate();
            } else if (from == buttons[4]) {
                clicked(buttons[4]);
                if (!hasBookedAppointment(patientID)) {
                    JOptionPane.showMessageDialog(this, "You haven't booked an appointment!!!", "Error", JOptionPane.ERROR_MESSAGE);
                } else {
                    right.removeAll();
                    right.repaint();
                    right.revalidate();
                    right.add(new MakePaymentPanel(patientID, firstName));
                    right.repaint();
                    right.revalidate();
                }
            }
    ...
    public void makePaymentFunction(int patientID){
        right.removeAll();
        right.repaint();
        right.revalidate();
        right.add(new MakePaymentPanel(patientID,firstName));
    ...
    public JPanel Top(String firstName) {
        JPanel top = new JPanel();
        top.setBounds(0, 0, width, height / 7);
        top.setBackground(new Color(3, 19, 255, 255));
        top.setLayout(null);
        Image icon1 = Toolkit.getDefaultToolkit().getImage("C:\\Users\\imma_\\IdeaProjects\\JJ\\src\\IMG-20240603-WA0021.jpg").getScaledInstance(75, 75, Image.SCALE_SMOOTH);
        ImageIcon icone1 = new ImageIcon(icon1);
        JLabel topdisplay1 = new JLabel("", icone1, JLabel.CENTER);
        topdisplay1.setBounds(20, 20, 80, 80);
        top.add(topdisplay1);
        JLabel username = new JLabel("Welcome " + firstName);
        username.setBounds(110, 30, 300, 75);
